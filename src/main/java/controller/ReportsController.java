...
public class ReportsController extends HttpServlet{
        ...
        if(!Servlet.isLogged(req)){
            ...
            if (operation == 1) {
                ...
                if (operation == 5){
                    ...
                    //guests
                    LocalDateTime now = LocalDateTime.now().withNano(0);
                    String reportName = ("HostsReport_"+now+".pdf").trim().replaceAll(":","-").replaceAll("T","_");
                    HashMap params = new HashMap();
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        if(!Servlet.isLogged(req)){
            ...
            if (operation == 2){
                LocalDateTime now = LocalDateTime.now().withNano(0);
                String reportName = ("HallsReport_"+now+".pdf").trim().replaceAll(":","-").replaceAll("T","_");
                HashMap params = new HashMap();
                ...
                LocalDateTime now = LocalDateTime.now().withNano(0);
                String reportName = ("HallTypesReport_"+now+".pdf").trim().replaceAll(":","-");
                HashMap params = new HashMap();
                ...
                LocalDateTime now = LocalDateTime.now().withNano(0);
                String reportName = ("OccupancyReport_"+now+".pdf").trim().replaceAll(":","-").replaceAll("T","_");
                HashMap params = new HashMap();
                Date initialDate = (req.getParameter("initial") != "") ? Date.valueOf(req.getParameter("initial")) : null;
                Date finalDate = (req.getParameter("final") != "") ? Date.valueOf(req.getParameter("final")) : null;
                params.put("initial", initialDate );
